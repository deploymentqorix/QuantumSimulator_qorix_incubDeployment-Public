    ...
    for (const auto& op : circuit.get_ops()) {
        std::visit([&](auto&& arg) {
            ...
            if constexpr (std::is_same_v<T, GateOp>) {
                ...
                if (arg.name == "h") {
                    apply_single_qubit(state_vector, arg.qubits[0], Gates::Hadamard);
                } else if (arg.name == "x") {
                    apply_single_qubit(state_vector, arg.qubits[0], Gates::PauliX);
                } else if (arg.name == "cx") {
                    apply_cnot(state_vector, arg.qubits[0], arg.qubits[1]);
                } else if (arg.name == "cz") {
                    apply_cz(state_vector, arg.qubits[0], arg.qubits[1]);
                } else if (arg.name == "swap") {
                    apply_swap(state_vector, arg.qubits[0], arg.qubits[1]);
                }
                ...
                for (const auto& fused_op : arg.ops) {
                    if (fused_op.name == "h") {
                        apply_single_qubit(state_vector, fused_op.qubits[0], Gates::Hadamard);
                    } else if (fused_op.name == "x") {
                        apply_single_qubit(state_vector, fused_op.qubits[0], Gates::PauliX);
                    }
